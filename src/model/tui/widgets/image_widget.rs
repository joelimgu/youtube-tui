...
impl Image {
    ...
    pub fn print_img(img: RgbImage, area: Rect) {
        let size = (area.height - 1, area.width - 1);
        let initial_position = (area.x, area.y);
        let scale = Image::calculate_scale(&img, size);
    ...
    fn calculate_scale(img: &RgbImage, size: (u16, u16)) -> (i32, i32) {
...
impl Widget for Image {
    fn render(self, area: Rect, _buf: &mut Buffer) {
        Image::print_img(self.image, area)
    }
}
